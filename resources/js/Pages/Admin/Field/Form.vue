...
const title = computed(() => isEdit.value ? 'Edit Lapangan' : 'Tambah Lapangan')
const submitLabel = computed(() => isEdit.value ? 'Simpan Perubahan' : 'Simpan Lapangan')
...
onMounted(() => {
    if (props.field) {
        ...
        form.surface_type = props.field.surface_type
        form.roof_type = props.field.roof_type
        form.player_capacity = props.field.player_capacity
        ...
        <div class="p-4 md:p-6">
            <div class="bg-white rounded-lg shadow overflow-hidden max-w-4xl mx-auto">
                <div class="px-6 py-4 border-b border-secondary/20 flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-dark">{{ isEdit ? 'Form Edit Lapangan' : 'Form Tambah Lapangan' }}</h3>
                ...
                <form @submit.prevent="submit" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div v-if="existingPhotos.length > 0" class="mb-4">
                            <h4 class="text-xs font-medium text-gray-500 mb-2 uppercase tracking-wider">Foto Saat Ini</h4>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div v-for="(photo, index) in existingPhotos" :key="photo.id" class="relative group">
                                    <img :src="'/' + photo.photo_url" class="h-24 w-full object-cover rounded-lg border border-gray-200" />
                                    <button type="button" @click="removeExistingPhoto(index, photo.id)" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow hover:bg-red-600 transition-colors" title="Hapus foto">
                        ...
                        <div v-if="photoPreviews.length > 0" class="mt-4">
                             <h4 class="text-xs font-medium text-gray-500 mb-2 uppercase tracking-wider">Foto Baru</h4>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div v-for="(preview, index) in photoPreviews" :key="index" class="relative group">
                                    ...
                                    <button type="button" @click="removeNewPhoto(index)" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow hover:bg-red-600 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                    </button>
                                </div>
                            </div>
